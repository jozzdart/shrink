  ...
  group('Unique List Compression Tests', () {
    ...
    test('shrinkUnique and restoreUnique preserve data with various test cases',
      ...
      final testCases = [
        ...
        ListTestDataGenerator.generateChunkedList(
            size: 1000, chunkSize: 10, gapRatio: 5.0),
    ...
    test('shrinkUnique compresses better than individual methods in some cases',
      ...
      final mixedTestCase = [
        ...
        ...List.generate(
            100, (i) => 1000 + i * 25), // Sparse (good for DeltaVarint)
        ...List.generate(
            100, (i) => 10000 + (i ~/ 10) * 256 + (i % 10)), // Chunked pattern
      ];
      ...
      final methodSizes = {
        ...
        'DeltaVarint': shrinkUniqueManual(
                mixedTestCase, UniqueCompressionMethod.deltaVarint)
            .length,
        'RunLength':
            shrinkUniqueManual(mixedTestCase, UniqueCompressionMethod.runLength)
                .length,
        'Chunked':
            shrinkUniqueManual(mixedTestCase, UniqueCompressionMethod.chunked)
                .length,
        'Bitmask':
            shrinkUniqueManual(mixedTestCase, UniqueCompressionMethod.bitmask)
                .length,
      ...
      final bestIndividualSize = methodSizes.entries
          .where((e) => !['UniqueCompression', 'No shrink'].contains(e.key))
          .map((e) => e.value)
          .reduce((a, b) => a < b ? a : b);
    ...
    test('shrinkUnique performance with different data distributions', () {
      ...
      final distributions = [
        ...
        _DistributionInfo(
            'Sparse-Low',
            ListTestDataGenerator.generateSparseList(
                size: 1000, sparsity: 5.0)),
        _DistributionInfo(
            'Sparse-High',
            ListTestDataGenerator.generateSparseList(
                size: 1000, sparsity: 50.0)),
        _DistributionInfo(
            'Chunked-Small',
            ListTestDataGenerator.generateChunkedList(
                size: 1000, chunkSize: 5, gapRatio: 2.0)),
        _DistributionInfo(
            'Chunked-Large',
            ListTestDataGenerator.generateChunkedList(
                size: 1000, chunkSize: 50, gapRatio: 2.0)),
        _DistributionInfo(
            'Random',
            ListTestDataGenerator.generateSortedUniqueList(
    ...
    test('shrinkUnique with large integer values', () {
      ...
      final largeValueLists = [
        _DistributionInfo(
            'Large-Sequential', List.generate(1000, (i) => i + 1000000)),
        _DistributionInfo(
            'Large-Sparse',
            ListTestDataGenerator.generateSparseList(size: 1000, sparsity: 10.0)
                .map((i) => i + 1000000)
                .toList()),
        _DistributionInfo('VeryLarge-Values',
            List.generate(1000, (i) => i * 10000 + 1000000)),
      ];
    ...
    test('shrinkUnique with special patterns', () {
      ...
      final specialPatternLists = [
        ...
        _DistributionInfo(
            'Alternating', List.generate(1000, (i) => i % 2 == 0 ? 0 : 1)),
      ...
      for (final list in specialPatternLists) {
        ...
        final compressed = {
          ...
          'DeltaVarint': shrinkUniqueManual(
              list.data, UniqueCompressionMethod.deltaVarint),
          'RunLength':
              shrinkUniqueManual(list.data, UniqueCompressionMethod.runLength),
          'Chunked':
              shrinkUniqueManual(list.data, UniqueCompressionMethod.chunked),
          'Bitmask':
              shrinkUniqueManual(list.data, UniqueCompressionMethod.bitmask),
        };
    ...
    test('shrinkUnique with mixed compression-friendly datasets', () {
      ...
      final complexDatasets = [
        ...
        _DistributionInfo('Mixed-SeqSparse', (() {
          ...
          // Add sparse blocks
          result.addAll(ListTestDataGenerator.generateSparseList(
                  size: 500, sparsity: 50.0)
              .map((i) => i + 10000)
              .toList());
          return result..sort();
        })()),
        ...
        _DistributionInfo('Simulated-IDs', (() {
          ...
          // Some random IDs
          result.addAll(ListTestDataGenerator.generateSortedUniqueList(
              size: 1000, maxValue: 100000));
          return result..sort();
        })()),
      ...
      for (final dataset in complexDatasets) {
        ...
        final methodResults = <String, Uint8List>{
          ...
          'DeltaVarint': shrinkUniqueManual(
              dataset.data, UniqueCompressionMethod.deltaVarint),
          'RunLength': shrinkUniqueManual(
              dataset.data, UniqueCompressionMethod.runLength),
          'Chunked':
              shrinkUniqueManual(dataset.data, UniqueCompressionMethod.chunked),
          'Bitmask':
              shrinkUniqueManual(dataset.data, UniqueCompressionMethod.bitmask),
        };
    ...
    test('shrinkUnique with extremely large lists', () {
      ...
      final largeLists = [
        _DistributionInfo(
            'Huge-Sequential', List.generate(50000, (i) => i).toSet().toList()),
        _DistributionInfo(
            'Huge-Random',
            ListTestDataGenerator.generateSortedUniqueList(
                size: 50000, maxValue: 1000000)),
        _DistributionInfo(
            'Huge-Sparse',
            ListTestDataGenerator.generateSparseList(
                size: 25000, sparsity: 100.0)),
        _DistributionInfo(
            'Huge-Chunked',
            ListTestDataGenerator.generateChunkedList(
                size: 25000, chunkSize: 200, gapRatio: 3.0)),
      ];
...
Map<UniqueCompressionMethod, int> _getCompressionSizes(List<int> ids) {
  ...
  return {
    UniqueCompressionMethod.deltaVarint:
        shrinkUniqueManual(sortedIds, UniqueCompressionMethod.deltaVarint)
            .length,
    UniqueCompressionMethod.runLength:
        shrinkUniqueManual(sortedIds, UniqueCompressionMethod.runLength).length,
    UniqueCompressionMethod.chunked:
        shrinkUniqueManual(sortedIds, UniqueCompressionMethod.chunked).length,
    UniqueCompressionMethod.bitmask:
        shrinkUniqueManual(sortedIds, UniqueCompressionMethod.bitmask).length,
  };
}
